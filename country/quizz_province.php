...
<body>
    ...
    <div id="main" style="margin-left:10%">
        ...
        <table><thead><tr><td>Pays</td><td>Style de carte</td><td>Timer</td><td>Label province</td><td>Label province 2</td></tr></thead>
        <tbody><tr>
        ...
        <td>La touche F permet d'activer/désactiver le plein ecran.</td>
        </tr></tbody></table>
...
<script>
  ...
  var cList = [
    {country : "Japon", geojson : "japan_prefectures.geojson"},
    {country : "Japon (kanji)", geojson : "japan_prefectures_kanji.geojson"}
];
  showCountries();
...
<script>
      ...
      function onClick(e) { 
        ...
        if (isOngoing) {
            if ( e.target.feature.properties.province == quizzAnswer) {
                      soundClick.play();
                      ...
                      if (hasLocation || hasAreaCode){
                        let str = "";
                        if (hasAreaCode) {str += e.target.feature.properties.province+" ";}
                        if (hasLocation) {str += e.target.feature.properties.province2;}
          ...
          async function loadJSON(oPays) {
              ...
              quizzList = new Array();
              json.features.forEach (element =>quizzList.push(element.properties.province));
              if (geoLayer != undefined) { geoLayer.remove(mapCities);}
                                          geoLayer = L.geoJSON(json,
                                            { onEachFeature: function (feature, layer) {
                                              layerList[feature.properties.province] = layer;
      ...
      layerList = new Array();
              ...
              if (geoLayer != undefined) { geoLayer.remove(mapCities);}
                                          geoLayer = L.geoJSON(json,
                                            { onEachFeature: function (feature, layer) {
                                              layerList[feature.properties.province] = layer;
                                              layer.bindTooltip(feature.properties.province2+" "+feature.properties.province,{permanent: true}).openTooltip();
                                              layer.setStyle({ fillColor: 'white', weight: 2, opacity: 1,color: 'black',fillOpacity: 0.1});
